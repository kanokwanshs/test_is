...
#     if len(X) >= 10 and 'potential_profit_increase_%' in price_ml_data.columns:
...
#     # Churn insights - ✅ เพิ่มการตรวจสอบว่ามีคอลัมน์ ai_risk_category หรือไม่
#     if 'ai_risk_category' in rfm.columns:
...
#     else:
#         insights.append("ℹ️ <b>Churn Analysis:</b> Need more data to train AI churn prediction model (minimum 20 customers)")

#     # Feature importance insights - Price Model
...
#     # Feature importance insights - Churn Model
#     if 'churn_importance' in locals() and not churn_importance.empty:
...
#     if 'mae' in locals():
#         insights.append(f"📈 <b>Price Model:</b> Achieves {(1 - mae/y_test.mean())*100:.1f}% accuracy in price predictions")

#     # ✅ เพิ่ม fallback message ถ้าไม่มี insights
#     if not insights:
#         insights.append("ℹ️ <b>Getting Started:</b> Upload more data to unlock AI-powered insights and predictions")
...
# ========== Class AIEstimator ==========
class AIEstimator:
    """AI-powered estimation for missing data with improved formulas"""
    
    @staticmethod
    def estimate_accounts_receivable(df_filtered, monthly_fin):
        """
        ปรับปรุง: ใช้ DSO (Days Sales Outstanding) แทนการคูณ revenue ตรงๆ
        AR = (Total Revenue / Days in Period) × Average Collection Days
        """
        total_revenue = df_filtered['net_revenue'].sum()
        
        # คำนวณจำนวนวันในช่วงข้อมูล
        date_range = (df_filtered['order_date'].max() - df_filtered['order_date'].min()).days
        days_in_period = max(date_range, 1)  # ป้องกันหาร 0
        
        # Average daily revenue
        avg_daily_revenue = total_revenue / days_in_period
        
        # Method 1: มีข้อมูล customer_type
        if 'customer_type' in df_filtered.columns:
            b2b_orders = df_filtered[df_filtered['customer_type'] == 'B2B']
            b2c_orders = df_filtered[df_filtered['customer_type'] == 'B2C']
            
            b2b_revenue = b2b_orders['net_revenue'].sum()
            b2c_revenue = b2c_orders['net_revenue'].sum()
            
            # B2B: เครดิต 30-45 วัน, B2C: ชำระทันที 0-3 วัน
            b2b_collection_days = 37.5  # เฉลี่ย 30-45
            b2c_collection_days = 1.5   # เฉลี่ย 0-3
            
            # คำนวณ AR แยกตาม customer type
            b2b_daily_revenue = b2b_revenue / days_in_period if days_in_period > 0 else 0
            b2c_daily_revenue = b2c_revenue / days_in_period if days_in_period > 0 else 0
            
            estimated_ar = (b2b_daily_revenue * b2b_collection_days) + \
                          (b2c_daily_revenue * b2c_collection_days)
            
            method = f"DSO-based: B2B {b2b_collection_days}d, B2C {b2c_collection_days}d"
        
        # Method 2: ไม่มีข้อมูล customer_type → ใช้ order concentration
        else:
            order_concentration = df_filtered.groupby('order_id')['net_revenue'].sum()
            avg_order_value = order_concentration.mean()
            
            # ถ้า avg order value สูง → มีแนวโน้มเป็น B2B
            if avg_order_value > 5000:  # ปรับตามธุรกิจ
                collection_days = 30  # B2B pattern
                method = "High avg order (฿{:,.0f}) → B2B pattern: 30 days DSO".format(avg_order_value)
            elif avg_order_value > 2000:
                collection_days = 15  # Mixed
                method = "Medium avg order (฿{:,.0f}) → Mixed: 15 days DSO".format(avg_order_value)
            else:
                collection_days = 3   # B2C pattern
                method = "Low avg order (฿{:,.0f}) → B2C pattern: 3 days DSO".format(avg_order_value)
            
            estimated_ar = avg_daily_revenue * collection_days
        
        # Growth adjustment (ถ้ามีข้อมูลหลายเดือน)
        if len(monthly_fin) >= 3:
            recent_growth = monthly_fin['net_revenue'].pct_change().tail(3).mean()
            if not np.isnan(recent_growth) and recent_growth > 0:
                growth_factor = min(1 + recent_growth, 1.5)  # จำกัดไม่เกิน 50%
                estimated_ar *= growth_factor
                method += f" + growth ({recent_growth*100:+.1f}%)"
        
        # ป้องกันค่าติดลบหรือมากเกินไป
        estimated_ar = max(0, min(estimated_ar, total_revenue * 0.5))
    ...
    def estimate_accounts_payable(cogs, df_filtered):
        """
        ปรับปรุง: ใช้ DPO (Days Payable Outstanding) แทนการคูณ COGS ตรงๆ
        AP = (COGS / Days in Period) × Average Payment Days
        """
        total_orders = df_filtered['order_id'].nunique()
        total_quantity = df_filtered['quantity'].sum()
        
        # คำนวณ average daily COGS
        date_range = (df_filtered['order_date'].max() - df_filtered['order_date'].min()).days
        days_in_period = max(date_range, 1)
        avg_daily_cogs = cogs / days_in_period
        
        # กำหนด payment terms ตามขนาดธุรกิจ
        if total_orders > 1000:
            payment_days = 45  # ธุรกิจใหญ่ มี leverage ต่อรองได้
            method = "Large business (>1000 orders): 45 days DPO"
        elif total_orders > 500:
            payment_days = 35
            method = "Medium business (500-1000): 35 days DPO"
        else:
            payment_days = 25  # ธุรกิจเล็ก จ่ายเร็วกว่า
            method = "Small business (<500): 25 days DPO"
        
        estimated_ap = avg_daily_cogs * payment_days
        
        # Sanity check: AP ไม่ควรเกิน 50% ของ COGS
        estimated_ap = min(estimated_ap, cogs * 0.5)
    ...
    def estimate_inventory(df_filtered, cogs):
        """
        ปรับปรุง: ใช้ Inventory Turnover Ratio
        ...
        avg_product_cost = cogs / total_quantity
        
        # วิเคราะห์ product velocity
        product_sales = df_filtered.groupby('product_id')['quantity'].sum()
        fast_movers = len(product_sales[product_sales > product_sales.median()])
        slow_movers = product_count - fast_movers
        ...
        date_range = (df_filtered['order_date'].max() - df_filtered['order_date'].min()).days
        days_in_period = max(date_range, 30)  # ขอต่ำสุด 30 วัน
        daily_sales_qty = total_quantity / days_in_period
        
        # กำหนด inventory holding period ตาม product mix
        if slow_movers > fast_movers * 2:
            # สินค้าขายช้าเยอะ
            holding_days = 75
            method = f"Slow-moving heavy ({slow_movers}/{product_count} products): 75 days"
        elif fast_movers > slow_movers * 2:
            # สินค้าขายเร็วเยอะ
            holding_days = 35
            method = f"Fast-moving heavy ({fast_movers}/{product_count} products): 35 days"
        else:
            # ผสมพอๆ กัน
            holding_days = 55
            method = f"Mixed velocity ({fast_movers}F/{slow_movers}S): 55 days"
        
        # เพิ่ม safety stock (10-15% ของ holding period)
        safety_buffer = 1.12  # +12% safety stock
        
        # คำนวณมูลค่า inventory
        estimated_inventory_units = daily_sales_qty * holding_days * safety_buffer
        estimated_inventory_value = estimated_inventory_units * avg_product_cost
        
        # Sanity check: Inventory ไม่ควรเกิน COGS 6 เดือน
        max_inventory = cogs * 0.5  # 6 เดือนคือ 50% ของ annual COGS
        if estimated_inventory_value > max_inventory:
            estimated_inventory_value = max_inventory
            method += " (capped at 6 months COGS)"
        
        return estimated_inventory_value, method
    
    @staticmethod
    def validate_estimation_quality(df_filtered):
        """
        เพิ่มใหม่: ตรวจสอบคุณภาพข้อมูลก่อน estimate
        Returns: (is_valid, issues_list)
        """
        issues = []
        
        # ตรวจสอบช่วงเวลาข้อมูล
        date_range = (df_filtered['order_date'].max() - df_filtered['order_date'].min()).days
        if date_range < 30:
            issues.append(f"⚠️ Data only covers {date_range} days (recommend 30+ days)")
        
        # ตรวจสอบจำนวน transactions
        total_orders = df_filtered['order_id'].nunique()
        if total_orders < 50:
            issues.append(f"⚠️ Only {total_orders} orders (recommend 50+ for accuracy)")
        
        # ตรวจสอบ negative values
        if (df_filtered['quantity'] < 0).any():
            issues.append("❌ Found negative quantities")
        
        if (df_filtered['net_revenue'] < 0).any():
            negative_count = (df_filtered['net_revenue'] < 0).sum()
            issues.append(f"⚠️ Found {negative_count} negative revenue entries (returns?)")
        
        # ตรวจสอบ missing critical data
        critical_columns = ['order_id', 'product_id', 'quantity', 'net_revenue', 'order_date']
        for col in critical_columns:
            if col in df_filtered.columns:
                missing_pct = df_filtered[col].isna().sum() / len(df_filtered) * 100
                if missing_pct > 5:
                    issues.append(f"⚠️ {col} has {missing_pct:.1f}% missing values")
...
def calculate_financial_metrics_with_validation(df_filtered, monthly_fin, quality_tracker):
    """
    คำนวณเมตริกทางการเงินพร้อมตรวจสอบความถูกต้อง
    """
    # Validate data quality first
    is_valid, issues = AIEstimator.validate_estimation_quality(df_filtered)
    ...
    # Calculate COGS
    if 'cogs_per_unit' in df_filtered.columns and df_filtered['cogs_per_unit'].notna().any():
        total_cogs = (df_filtered['quantity'] * df_filtered['cogs_per_unit']).sum()
        quality_tracker.mark_actual('COGS')
        cogs_method = "Actual COGS from data"
    else:
        total_revenue = df_filtered['net_revenue'].sum()
        total_cogs = total_revenue * 0.50
        quality_tracker.mark_estimated('COGS', '50% of revenue (industry avg)')
        cogs_method = "Estimated (50% of revenue)"
    
    # Calculate AR
    if 'accounts_receivable' in df_filtered.columns and df_filtered['accounts_receivable'].notna().any():
        ar = df_filtered['accounts_receivable'].iloc[-1]
        quality_tracker.mark_actual('Accounts Receivable')
        ar_method = "Actual AR from data"
    else:
        ar, ar_method = AIEstimator.estimate_accounts_receivable(df_filtered, monthly_fin)
        quality_tracker.mark_estimated('Accounts Receivable', ar_method)
    
    # Calculate AP
    if 'accounts_payable' in df_filtered.columns and df_filtered['accounts_payable'].notna().any():
        ap = df_filtered['accounts_payable'].iloc[-1]
        quality_tracker.mark_actual('Accounts Payable')
        ap_method = "Actual AP from data"
    else:
        ap, ap_method = AIEstimator.estimate_accounts_payable(total_cogs, df_filtered)
        quality_tracker.mark_estimated('Accounts Payable', ap_method)
    
    # Calculate Inventory
    if 'inventory_value' in df_filtered.columns and df_filtered['inventory_value'].notna().any():
        inventory = df_filtered['inventory_value'].iloc[-1]
        quality_tracker.mark_actual('Inventory')
        inventory_method = "Actual inventory from data"
    else:
        inventory, inventory_method = AIEstimator.estimate_inventory(df_filtered, total_cogs)
        quality_tracker.mark_estimated('Inventory', inventory_method)
    ...
    # Working Capital
    cash = df_filtered['cash'].iloc[-1] if 'cash' in df_filtered.columns else 0
    working_capital = (cash + ar + inventory) - ap
